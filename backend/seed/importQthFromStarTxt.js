...
const SOURCE_FILE = path.resolve(__dirname, "..", "..", "monhoc", "qth.txt");
const REPORT_FILE = path.resolve(__dirname, "reports", "qth_import_qa.csv");
const SUBJECT_CODE = "QTH";
...
const addQuestionBreaks = (text) => {
  // Extracted/typed source can place next "Câu x" on the same line as previous option.
  return String(text || "")
    .replace(/([^\n])\s+(Câu\s*\d+\s*[:;.\-]?)/gi, "$1\n$2");
};
...
const parseQuestionsInLesson = (lessonBlock) => {
  ...
  for (const chunk of chunks) {
    ...
    let starCount = 0;
    const answers = optionParts
      .map((raw) => {
        const hasStar = /\*/.test(raw);
        if (hasStar) starCount += 1;
        const withoutLabel = raw.replace(/^\s*[A-D]\s*[\.\):]\s*/i, "");
        return {
          text: withoutLabel.replace(/\*/g, "").replace(/\s+/g, " ").trim(),
          imageUrl: "",
          isCorrect: hasStar,
        };
      })
...
const run = async () => {
  ...
  if (!lessonNos.length) {
    throw new Error("Không tách được bài học nào từ file nguồn qth.txt");
  }
  ...
  console.log(DRY_RUN ? "[DRY RUN] Import QTH từ file có đánh dấu *" : "Import QTH từ file có đánh dấu *");
